...
6人・宝石5個・ウェイト `[10, 15, 2, 1, 1, 1]`・ランダム順・1000エポックでの検証結果
（詳細は `result/result_1.txt` 〜 `result_4.txt`）。

- **n-stepリターンの窓はエピソード最大長以上が必須**（`run_experiment.py` で自動設定）。
  AECでは提案から否決確定（$-L$）までバッファ上で最大「生存者数」ステップ離れるため、
  `n_step=3` のままだと終端報酬が提案・序盤投票の遷移に届かず学習が停止する
  （試行1・2で確認。提案可決率が10%前後から改善しなかった）。
- **L（命の重さ）が政治レジームを制御する**:
